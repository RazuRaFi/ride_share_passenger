...
class CreateLoadMapController extends GetxController {
  ...
  Future<String> onMapTapped(LatLng tappedLocation, {bool isSearched = false}) async {
    selectedLocation.value = tappedLocation;
    // FindLoadController.instance.setLocation(locationLatitude: tappedLocation.latitude, locationLongitude: tappedLocation.longitude);
    ...
    if(isSearched){
      await getAddress(latitude: tappedLocation.latitude, longitude: tappedLocation.longitude);
    }else{
  ...
  updateAddresses(double latitude, double longitude, {bool isSearched = false}) async {
    await onMapTapped(LatLng(latitude, longitude), isSearched: isSearched);
  }
